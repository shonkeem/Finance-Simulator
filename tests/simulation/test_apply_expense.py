...
non_inflation_expense = ExpenseLoad(name="test_expense", monthly_amount=50, category="test_category", inflation_linked=False)
non_inflation_settings = SettingsInput(inflation_rate=0.02, apply_inflation_to_expenses=False)

def test_non_inflation_expense():
    new_state = apply_expense(state=prev_state, load=non_inflation_expense, settings=non_inflation_settings, start_date=start_date)
    assert new_state.cash == 50
    assert new_state.expenses == 50

inflation_expense = ExpenseLoad(name="test_expense", monthly_amount=50, category="test_category", inflation_linked=True)
inflation_settings = SettingsInput(inflation_rate=0.02, apply_inflation_to_expenses=True)

def test_inflation_expense():
    new_state = apply_expense(state=prev_state, load=inflation_expense, settings=inflation_settings, start_date=start_date)
